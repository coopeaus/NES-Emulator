...
template <int quality, int range> class Blip_Synth
{
  ...
  static constexpr int fine_bits = ( fine_mode ? ( abs_range <= 64     ? 2
                                                   : abs_range <= 128  ? 3
                                                   : abs_range <= 256  ? 4
                                                   : abs_range <= 512  ? 5
                                                   : abs_range <= 1024 ? 6
...
public:
  ...
  void offset_resampled( blip_resampled_time_t t, int o ) const { offset_resampled( t, o, impulse.buf ); }
  ...
  void offset_inline( blip_time_t time, int delta ) const { offset_inline( time, delta, impulse.buf ); }
};
...
template <int quality, int range> void Blip_Wave<quality, range>::amplitude( int amp )
{
