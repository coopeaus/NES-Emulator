...
{
  unsigned new_size = ( std::numeric_limits<unsigned>::max() >> BLIP_BUFFER_ACCURACY ) + 1u - widest_impulse_ - 64u;
...
{
  if ( !generate && new_eq.treble == eq.treble && new_eq.cutoff == eq.cutoff && new_eq.sample_rate == eq.sample_rate )
  ...
  for ( int i = size; i--; ) {
    ...
    double c = pow_a_n * rolloff * cos( ( n_harm - 1.0 ) * angle ) - pow_a_n * cos( n_harm * angle ) -
