...
private:
  ...
  void RenderPpuPaletteGrid( ImVec2 cellSize )
  {
    ...
    for ( int rowStart = 0; rowStart < 32; rowStart += 4 ) {
      ...
      for ( int cell = 0; cell < 4; cell++ ) {
        ...
        u8 const     colorIndex = renderer->bus.ppu.ReadVram( paletteAddress );
        ImVec4 const paletteColor = Rgba32ToImVec4( renderer->bus.ppu.GetMasterPaletteColor( colorIndex ) );
        char         label[3];
  ...
  {
    float const luminance = static_cast<float>( ( 0.299 * color.x ) + ( 0.587 * color.y ) + ( 0.114 * color.z ) );
  ...
  void RenderSystemPaletteGrid( ImVec2 cellSize )
  {
    ...
    for ( int rowStart = 0; rowStart < 64; rowStart += 8 ) {
      ...
      for ( int cell = 0; cell < 8; cell++ ) {
        ...
        ImVec4 const paletteColor = Rgba32ToImVec4( renderer->bus.ppu.GetMasterPaletteColor( rowStart + cell ) );
