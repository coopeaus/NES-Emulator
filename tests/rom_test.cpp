...
TEST( RomTests, Nestest )
{
    ...
    std::ofstream actualOutput( "tests/output/my_nestest-log.txt" );
    ...
    if ( !actualOutput.is_open() ) {
        std::cerr << "Failed to open output/my_nestest-log.txt for writing\n";
    ...
    while ( lineIndex < expectedLines.size() ) {
        try {
            ...
            if ( cpu.GetProgramCounter() != expected.pc ) {
                didFail = true;
                std::cerr << '\n';
                std::cerr << red << ( lineIndex + 1 ) << ": PC mismatch" << reset << '\n';
                std::cerr << "Expected: " << utils::toHex( expected.pc, 4 ) << "    "
            ...
            if ( didFail ) {
                std::string    actualLine = cpu.LogLineAtPC();
                ...
                std::string expectedLine = cpu.LogLineAtPC();
