...
void Nes_Dmc::recalc_irq()
{
  ...
  if ( irq_enabled && length_counter )
    irq = apu->last_time + delay + ( ( length_counter - 1 ) * 8 + bits_remain - 1 ) * cpu_time_t( period ) + 1;
  if ( irq != next_irq ) {
    next_irq = irq;
    apu->irq_changed();
  }
...
static const unsigned char dac_table[128] = {
    0,  0,  1,  2,  2,  3,  3,  4,  5,  5,  6,  7,  7,  8,  8,  9,  10, 10, 11, 11, 12, 13, 13, 14, 14, 15,
    15, 16, 17, 17, 18, 18, 19, 19, 20, 20, 21, 21, 22, 22, 23, 23, 24, 24, 25, 25, 26, 26, 27, 27, 28, 28,
    29, 29, 30, 30, 31, 31, 32, 32, 32, 33, 33, 34, 34, 35, 35, 35, 36, 36, 37, 37, 38, 38, 38, 39, 39, 40,
    40, 40, 41, 41, 42, 42, 42, 43, 43, 44, 44, 44, 45, 45, 45, 46, 46, 47, 47, 47, 48, 48, 48, 49, 49, 49,
...
static const short noise_period_table[16] = { 0x004, 0x008, 0x010, 0x020, 0x040, 0x060, 0x080, 0x0A0,
