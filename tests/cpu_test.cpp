...
TEST_F( CPUTestFixture, StatusFlags )
{
    ...
    constexpr u8 interruptDisable = 0b00000100;
    constexpr u8 decimal = 0b00001000;
    constexpr u8 breakFlag = 0b00010000;
    ...
    EXPECT_EQ( cpu.GetStatusRegister(), 0x00 | carry | zero | interruptDisable | unused );
    SetFlags( decimal );
    EXPECT_EQ( cpu.GetStatusRegister(), 0x00 | carry | zero | interruptDisable | decimal | unused );
    SetFlags( breakFlag );
    EXPECT_EQ( cpu.GetStatusRegister(),
               0x00 | carry | zero | interruptDisable | decimal | breakFlag | unused );
    ClearFlags( carry | zero | interruptDisable | decimal | breakFlag | unused );
    ...
    // set all flags
    SetFlags( carry | zero | interruptDisable | decimal | breakFlag | overflow | negative | unused );
    EXPECT_EQ( cpu.GetStatusRegister(),
               0x00 | carry | zero | interruptDisable | decimal | breakFlag | overflow | negative | unused );
    // clear all flags
    ClearFlags( carry | zero | interruptDisable | decimal | breakFlag | overflow | negative | unused );
    EXPECT_EQ( cpu.GetStatusRegister(), 0x00 );
...
{
    std::string const testName = "Immediate Addressing Mode";
...
{
    std::string const testName = "Zero Page Addressing Mode";
    printTestStartMsg( testName );
...
{
    std::string const testName = "Zero Page X Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
    ...
    u8 const zeroPageAddr = Read( cpu.GetProgramCounter() );
    u8 const expectedAddr = ( zeroPageAddr + 0x1 ) & 0xFF;

    Write( expectedAddr, 0x42 ); // Write a test value at the expected address
    u16 const addr = ZPGX();
...
{
    std::string const testName = "Zero Page Y Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
...
{
    std::string const testName = "Absolute Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
    Write( 0x0000, 0x42 );
    Write( 0x0001, 0x24 );
...
{
    std::string const testName = "Absolute X Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
    ...
    constexpr u16 expectedAddr = 0x2480 + 0x10;
...
{
    std::string const testName = "Absolute Y Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
...
{
    std::string const testName = "Indirect Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
...
{
    std::string const testName = "Indirect Addressing Mode Bug";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
    Write( 0x0000, 0xFF );
    Write( 0x0001, 0x02 );
    ...
    constexpr u16 bugAddr = 0x1234;
    Write( bugAddr, 0xEF );

    constexpr u16 noBugAddr = 0x5634;
    Write( noBugAddr, 0xAB );

    u16 const effectiveAddr = IND();
    EXPECT_EQ( effectiveAddr, bugAddr ) << "Expected 0x1234, but got " << std::hex << effectiveAddr;
    EXPECT_EQ( Read( effectiveAddr ), 0xEF )
        << "Expected 0xEF, but got " << static_cast<int>( Read( effectiveAddr ) );
    EXPECT_EQ( cpu.GetProgramCounter(), 0x0002 );
    printTestEndMsg( testName );
}
...
{
    std::string const testName = "Indirect X Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
    cpu.SetXRegister( 0x10 );
    ...
    constexpr u16 effectiveAddr = 0xABCD;

    // Write the effective address to zero-page memory
    Write( ptr, effectiveAddr & 0xFF );
    Write( ( ptr + 1 ) & 0xFF, effectiveAddr >> 8 );

    // Write a test value at the effective address
    Write( effectiveAddr, 0x42 );

    u16 const addr = INDX();
    EXPECT_EQ( addr, effectiveAddr ) << "Expected " << std::hex << effectiveAddr << ", but got " << addr;
...
{
    std::string const testName = "Indirect Y Addressing Mode";
    printTestStartMsg( testName );
    cpu.SetProgramCounter( 0x0000 );
    cpu.SetYRegister( 0x10 );
    ...
    constexpr u16 effectiveAddr = 0xAB00;

    // Write the effective address to zero-page memory
    Write( ptr, effectiveAddr & 0xFF );
    Write( ptr + 1, effectiveAddr >> 8 );

    u16 const finalAddr = effectiveAddr + cpu.GetYRegister();
    Write( finalAddr, 0x42 ); // Write a test value at the final address
    u16 const addr = INDY();
...
{
    std::string const testName = "Relative Addressing Mode";
    cpu.SetProgramCounter( 0x1000 );

    // Write a relative address of -5 at the pc location
    Write( 0x1000, 0xFB );
    u16 const backBranch = REL();
    EXPECT_EQ( backBranch, 0x0FFC ) << "Expected 0x0FFC, but got " << std::hex << backBranch;
    EXPECT_EQ( cpu.GetProgramCounter(), 0x1001 ) << "Expected PC to be 0x1001 after REL";

    // Write a relative address of +5 at the pc location
    cpu.SetProgramCounter( 0x1000 );
    Write( 0x1000, 0x05 );
    u16 const forwardBranch = REL();
    EXPECT_EQ( forwardBranch, 0x1006 ) << "Expected 0x1006, but got " << std::hex << forwardBranch;
    EXPECT_EQ( cpu.GetProgramCounter(), 0x1001 ) << "Expected PC to be 0x1001 after REL";
