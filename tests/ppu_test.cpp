...
void PPUTestFixture::RunTestCase( const json &testCase )
{
    ...
    EXPECT_EQ(ppu._ppuCtrl.value, testCase["initial"]["ppuctrl"]);
    EXPECT_EQ(ppu._ppuMask.value, testCase["initial"]["ppumask"]);
    EXPECT_EQ(ppu._ppuStatus.value, testCase["initial"]["ppustatus"]);
    EXPECT_EQ(ppu._oamAddr, testCase["initial"]["oamaddr"]);
    EXPECT_EQ(ppu._vramAddr.value, testCase["initial"]["vramaddr"]);
