...
static char *getPathWithoutFinalSlash( char       *aoDestination, /* make sure it is allocated, use _MAX_PATH */
...
static void RGB2Hex( unsigned char const aRGB[3], char aoResultHexRGB[8] )
{
  if ( aoResultHexRGB ) {
    ...
    ( defined( __STDC_VERSION__ ) && __STDC_VERSION__ >= 199901L ) || defined( __clang__ )
...
char const *
tinyfd_getGlobalChar( char const *aCharVariableName ) /* to be called from C# (you don't need this in C or C++) */
{
                         ...
                         int         aValue ) /* to be called from C# (you don't need this in C or C++) */
{
  ...
  /* else if (!strcmp(aIntVariableName, "tinyfd_assumeGraphicDisplay")) { tinyfd_assumeGraphicDisplay = aValue; return
...
static wchar_t *getPathWithoutFinalSlashW( wchar_t       *aoDestination, /* make sure it is allocated, use _MAX_PATH */
...
static wchar_t *getLastNameW( wchar_t       *aoDestination, /* make sure it is allocated */
...
static void hiddenConsoleW( wchar_t const *aString, wchar_t const *aDialogTitle, int aInFront )
{
  ...
  if ( !CreateProcessW( NULL, (LPWSTR) aString, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &StartupInfo,
                        &ProcessInfo ) ) {
    return; /* GetLastError(); */
  }
...
int tinyfd_messageBoxW( wchar_t const *aTitle,      /* NULL or "" */
                        ...
                        int            aDefaultButton )        /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
  ...
  /*if (quoteDetectedW(aTitle)) return tinyfd_messageBoxW(L"INVALID TITLE WITH QUOTES", aMessage, aDialogType,
  aIconType, aDefaultButton); if (quoteDetectedW(aMessage)) return tinyfd_messageBoxW(aTitle, L"INVALID MESSAGE WITH
  QUOTES", aDialogType, aIconType, aDefaultButton);*/
                ...
                wchar_t const * aIconType)
{
                ...
                if (quoteDetectedW(aTitle)) return tinyfd_notifyPopupW(L"INVALID TITLE WITH QUOTES", aMessage,
aIconType); if (quoteDetectedW(aMessage)) return tinyfd_notifyPopupW(aTitle, L"INVALID MESSAGE WITH QUOTES", aIconType);
                           ...
                           wchar_t const *aDefaultInput ) /* L"" , if NULL it's a passwordBox */
{
  ...
  if ( quoteDetectedW( aDefaultInput ) )
    return tinyfd_inputBoxW( aTitle, aMessage,
                             L"INVALID DEFAULT_INPUT WITH QUOTES: use the GRAVE ACCENT \\x60 instead." );
...
wchar_t *tinyfd_saveFileDialogW( wchar_t const        *aTitle,                /* NULL or "" */
                                 ...
                                 wchar_t const        *aSingleFilterDescription )    /* NULL or "image files" */
{
  ...
  /*if (quoteDetectedW(aTitle)) return tinyfd_saveFileDialogW(L"INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile,
  aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription); if (quoteDetectedW(aDefaultPathAndOrFile)) return
  tinyfd_saveFileDialogW(aTitle, L"INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns, aFilterPatterns,
  aSingleFilterDescription); if (quoteDetectedW(aSingleFilterDescription)) return tinyfd_saveFileDialogW(aTitle,
  aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns, L"INVALID FILTER_DESCRIPTION WITH QUOTES"); for (i = 0;
  i < aNumOfFilterPatterns; i++)
  {
                  if (quoteDetectedW(aFilterPatterns[i])) return tinyfd_saveFileDialogW(L"INVALID FILTER_PATTERN WITH
  QUOTES: use the GRAVE ACCENT \\x60 instead.", aDefaultPathAndOrFile, 0, NULL, NULL);
  }*/
...
wchar_t *tinyfd_openFileDialogW( wchar_t const        *aTitle,                   /* NULL or "" */
                                 wchar_t const        *aDefaultPathAndOrFile,    /* NULL or "" */
                                 int                   aNumOfFilterPatterns,     /* 0 */
                                 wchar_t const *const *aFilterPatterns,          /* NULL or {"*.jpg","*.png"} */
                                 wchar_t const        *aSingleFilterDescription, /* NULL or "image files" */
                                 int aAllowMultipleSelects ) /* 0 or 1 ; -1 to free allocated memory and return */
{
  ...
  /*if (quoteDetectedW(aTitle)) return tinyfd_openFileDialogW(L"INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile,
  aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects); if
  (quoteDetectedW(aDefaultPathAndOrFile)) return tinyfd_openFileDialogW(aTitle, L"INVALID DEFAULT_PATH WITH QUOTES",
  aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects); if
  (quoteDetectedW(aSingleFilterDescription)) return tinyfd_openFileDialogW(aTitle, aDefaultPathAndOrFile,
  aNumOfFilterPatterns, aFilterPatterns, L"INVALID FILTER_DESCRIPTION WITH QUOTES", aAllowMultipleSelects); for (i = 0;
  i < aNumOfFilterPatterns; i++)
  {
                  if (quoteDetectedW(aFilterPatterns[i])) return tinyfd_openFileDialogW(L"INVALID FILTER_PATTERN WITH
  QUOTES: use the GRAVE ACCENT \\x60 instead.", aDefaultPathAndOrFile, 0, NULL, NULL, aAllowMultipleSelects);
  }*/
...
wchar_t *tinyfd_selectFolderDialogW( wchar_t const *aTitle,        /* NULL or "" */
...
wchar_t *tinyfd_colorChooserW( wchar_t const      *aTitle,         /* NULL or "" */
                               ...
                               unsigned char       aoResultRGB[3] )      /* { 0 , 0 , 0 } */
{
  ...
  /*if (quoteDetectedW(aTitle)) return tinyfd_colorChooserW(L"INVALID TITLE WITH QUOTES", aDefaultHexRGB, aDefaultRGB,
  aoResultRGB); if (quoteDetectedW(aDefaultHexRGB)) return tinyfd_colorChooserW(aTitle, L"INVALID DEFAULT_HEX_RGB WITH
  QUOTES: use the GRAVE ACCENT \\x60 instead.", aDefaultRGB, aoResultRGB);*/
...
static int messageBoxWinGui( char const *aTitle,      /* NULL or "" */
...
static int notifyWinGui( char const *aTitle,   /* NULL or "" */
...
static int inputBoxWinGui( char *aoBuff, char const *aTitle, /* NULL or "" */
                           char const *aMessage,             /* NULL or "" may NOT contain \n nor \t */
...
static char *saveFileDialogWinGui( char *aoBuff, char const *aTitle,         /* NULL or "" */
...
static char *openFileDialogWinGui( char const        *aTitle,                   /*  NULL or "" */
                                   char const        *aDefaultPathAndOrFile,    /*  NULL or "" */
                                   int                aNumOfFilterPatterns,     /* 0 */
                                   char const *const *aFilterPatterns,          /* NULL or {"*.jpg","*.png"} */
                                   char const        *aSingleFilterDescription, /* NULL or "image files" */
...
static char *selectFolderDialogWinGui( char *aoBuff, char const *aTitle, /*  NULL or "" */
...
static char *colorChooserWinGui( char const         *aTitle,         /* NULL or "" */
...
static int messageBoxWinConsole( char const *aTitle,      /* NULL or "" */
                                 char const *aMessage,    /* NULL or ""  may contain \n and \t */
                                 char const *aDialogType, /* "ok" "okcancel" "yesno" "yesnocancel" */
                                 char const *aIconType,   /* "info" "warning" "error" "question" */
                                 int aDefaultButton )     /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
...
static int inputBoxWinConsole( char *aoBuff, char const *aTitle, /* NULL or "" */
                               char const *aMessage,             /* NULL or "" may NOT contain \n nor \t */
                               char const *aDefaultInput )       /* "" , if NULL it's a passwordBox */
{
  char  lDialogString[MAX_PATH_OR_CMD];
...
static char *saveFileDialogWinConsole( char *aoBuff, char const *aTitle,   /* NULL or "" */
                                       char const *aDefaultPathAndOrFile ) /* NULL or "" */
{
  char  lDialogString[MAX_PATH_OR_CMD];
...
static char *openFileDialogWinConsole( char const *aTitle,                 /*  NULL or "" */
                                       char const *aDefaultPathAndOrFile ) /*  NULL or "" */
{
...
static char *selectFolderDialogWinConsole( char *aoBuff, char const *aTitle, /*  NULL or "" */
                                           char const *aDefaultPath )        /* NULL or "" */
{
  char  lDialogString[MAX_PATH_OR_CMD];
                       ...
                       char const *aDefaultInput ) /* "" , if NULL it's a passwordBox */
{
  ...
  if ( tfd_quoteDetected( aDefaultInput ) )
    return tinyfd_inputBox( aTitle, aMessage,
                            "INVALID DEFAULT_INPUT WITH QUOTES: use the GRAVE ACCENT \\x60 instead." );
...
char *tinyfd_saveFileDialog( char const        *aTitle,                /* NULL or "" */
                             char const        *aDefaultPathAndOrFile, /* NULL or "" */
                             int                aNumOfFilterPatterns,  /* 0 */
                             char const *const *aFilterPatterns,       /* NULL or {"*.jpg","*.png"} */
                             char const        *aSingleFilterDescription )    /* NULL or "image files" */
{
  ...
  if ( tfd_quoteDetected( aTitle ) )
    return tinyfd_saveFileDialog( "INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile, aNumOfFilterPatterns,
                                  aFilterPatterns, aSingleFilterDescription );
  if ( tfd_quoteDetected( aDefaultPathAndOrFile ) )
    return tinyfd_saveFileDialog( aTitle, "INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns, aFilterPatterns,
                                  aSingleFilterDescription );
  if ( tfd_quoteDetected( aSingleFilterDescription ) )
    return tinyfd_saveFileDialog( aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
                                  "INVALID FILTER_DESCRIPTION WITH QUOTES" );
  for ( i = 0; i < aNumOfFilterPatterns; i++ ) {
    if ( tfd_quoteDetected( aFilterPatterns[i] ) )
      return tinyfd_saveFileDialog( "INVALID FILTER_PATTERN WITH QUOTES: use the GRAVE ACCENT \\x60 instead.",
  ...
  if ( ( !tinyfd_forceConsole || !( GetConsoleWindow() || dialogPresent() ) ) &&
    ...
    p = saveFileDialogWinGui( lBuff, aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns,
...
char *tinyfd_openFileDialog( char const        *aTitle,                   /* NULL or "" */
                             char const        *aDefaultPathAndOrFile,    /* NULL or "" */
                             int                aNumOfFilterPatterns,     /* 0 */
                             char const *const *aFilterPatterns,          /* NULL or {"*.jpg","*.png"} */
                             char const        *aSingleFilterDescription, /* NULL or "image files" */
                             int                aAllowMultipleSelects )                  /* 0 or 1 */
{
  ...
  if ( !aFilterPatterns )
    aNumOfFilterPatterns = 0;
  if ( tfd_quoteDetected( aTitle ) )
    return tinyfd_openFileDialog( "INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile, aNumOfFilterPatterns,
                                  aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects );
  if ( tfd_quoteDetected( aDefaultPathAndOrFile ) )
    return tinyfd_openFileDialog( aTitle, "INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns, aFilterPatterns,
                                  aSingleFilterDescription, aAllowMultipleSelects );
  if ( tfd_quoteDetected( aSingleFilterDescription ) )
    return tinyfd_openFileDialog( aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
                                  "INVALID FILTER_DESCRIPTION WITH QUOTES", aAllowMultipleSelects );
  for ( i = 0; i < aNumOfFilterPatterns; i++ ) {
    if ( tfd_quoteDetected( aFilterPatterns[i] ) )
      return tinyfd_openFileDialog( "INVALID FILTER_PATTERN WITH QUOTES: use the GRAVE ACCENT \\x60 instead.",
                                    aDefaultPathAndOrFile, 0, NULL, NULL, aAllowMultipleSelects );
  }
...
char *tinyfd_colorChooser( char const         *aTitle,         /* NULL or "" */
                           ...
                           unsigned char       aoResultRGB[3] )      /* { 0 , 0 , 0 } */
{
  ...
  if ( tfd_quoteDetected( aDefaultHexRGB ) )
    return tinyfd_colorChooser( aTitle, "INVALID DEFAULT_HEX_RGB WITH QUOTES: use the GRAVE ACCENT \\x60 instead.",
...
static char *terminalName( void )
{
  ...
  if ( lTerminalName[0] == '*' ) {
    ...
    if ( tfd_isDarwin() ) {
      ...
      strcat( lTerminalName, " -e " );
      strcat( lTerminalName, lShellName );
    } else if ( *strcpy( lTerminalName, "gnome-terminal" ) && detectPresence( lTerminalName ) &&
...
static int graphicMode( void )
{
  return !( tinyfd_forceConsole && ( isTerminalRunning() || terminalName() ) ) &&
         ( getenvDISPLAY() || ( tfd_isDarwin() && ( !getenv( "SSH_TTY" ) || getenvDISPLAY() ) ) );
}
...
static int perlPresent( void )
{
  ...
  if ( lPerlPresent < 0 ) {
    ...
    if ( lPerlPresent ) {
      lIn =
          popen( "perl -MNet::DBus -e \"Net::DBus->session->get_service('org.freedesktop.Notifications')\" 2>&1", "r" );
...
void tinyfd_beep( void )
{
  ...
  if ( pactlPresent() ) {
    ...
    strcpy(
        lDialogString,
        "( speaker-test -t sine -f 440 > /dev/tty )& pid=$!;sleep .5; kill -9 $pid" ); /*.3 was too short for mac g3*/
                       ...
                       int         aDefaultButton )     /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
 ...
 -e 'else' -e 'return 0' -e 'end if' " );
    ...
    if ( ( tfd_kdialogPresent() == 2 ) && tfd_xpropPresent() ) {
      strcat( lDialogString,
              " --attach=$(xprop -root 32x '\t$0' _NET_ACTIVE_WINDOW | cut -f 2)" ); /* contribution: Paul Rouget */
    }
    ...
    if ( tfd_zenityPresent() ) {
      ...
      if ( ( tfd_zenity3Present() >= 4 ) && !getenv( "SSH_TTY" ) && tfd_xpropPresent() ) {
        strcat( lDialogString,
                " --attach=$(sleep .01;xprop -root 32x '\t$0' _NET_ACTIVE_WINDOW | cut -f 2)" ); /* contribution: Paul
                                                                                                    Rouget */
      }
    ...
    if ( ( tfd_zenity3Present() >= 3 ) ||
         ( !tfd_zenityPresent() && ( tfd_shellementaryPresent() || tfd_qarmaPresent() ) ) ) {
...
if res is False :\n\tprint(0)\n\
  ...
  } else if ( !gxmessagePresent() && !gmessagePresent() && !gdialogPresent() && !xdialogPresent() &&
              tkinter2Present() ) {
    if ( aTitle && !strcmp( aTitle, "tinyfd_query" ) ) {
...
'org.freedesktop.Notifications');" );
    ...
    sprintf( lDialogString + strlen( lDialogString ),
             "my \\$notificationId;\\$notificationId = \\$notificationsObject->Notify(shift, 0, '%s', '%s', '%s', [], "
             "{}, -1);\" ",
...
void tfd_messageBox( char const *aTitle, char const *aMessage, char const *aDialogType, char const *aIconType,
                     int *aiDefaultButton )
{
...
void tfd_saveFileDialog( char const *aTitle, char **aiDefaultPathAndFile, int const *aNumOfFilterPatterns,
                         char const *const *aFilterPatterns, char const *aSingleFilterDescription )
{
  ...
  lSavefile = tinyfd_saveFileDialog( aTitle, *aiDefaultPathAndFile, *aNumOfFilterPatterns, aFilterPatterns,
                                     aSingleFilterDescription );
  if ( lSavefile )
...
void tfd_openFileDialog( char const *aTitle, char **aiDefaultPathAndFile, int const *aNumOfFilterPatterns,
                         char const *const *aFilterPatterns, char const *aSingleFilterDescription,
                         int const *aAllowMultipleSelects )
{
  ...
  lOpenfile = tinyfd_openFileDialog( aTitle, *aiDefaultPathAndFile, *aNumOfFilterPatterns, aFilterPatterns,
                                     aSingleFilterDescription, *aAllowMultipleSelects );
