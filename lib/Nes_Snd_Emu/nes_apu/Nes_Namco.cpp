...
void Nes_Namco::run_until( cpu_time_t nes_end_time )
{
  ...
  for ( int i = osc_count - active_oscs; i < osc_count; i++ ) {
    ...
    Blip_Buffer::resampled_time_t time = output->resampled_time( last_time ) + osc.delay;
    ...
    if ( time < end_time ) {
      ...
      if ( !freq )
        continue;
      Blip_Buffer::resampled_time_t period = output->resampled_duration( 983040 ) / freq * active_oscs;
